...
def adm_all_course_score(request):
    ...
    if request.is_ajax():
        if request.method == 'GET':
            ...
            print ("change")
            # all_course_score=CourseScore.objects.filter(teaching__mcno__year=year, teaching__mcno__semester=semester)
            all_course_score = CourseScore.objects.all()
            # all_course_score = list(all_course_score)
            all_course_score = serializers.serialize("json", CourseScore.objects.filter(teaching__mcno__year=year,teaching__mcno__semester=semester)[:3])
            # serializers.deserialize()
            print(len(all_course_score))
            context = {"all_course_score": all_course_score}
            return JsonResponse(context)
    else:
        all_course_score = CourseScore.objects.all()[0:100]
        context = {"all_course_score": all_course_score}
        print("HHHHHHH")
        return render(request, 'scoreManage/adm_score_manage.html', {"all_course_score": all_course_score})
   # else:
      #  return Http404()
...
def delete_score(request):
    ...
    year = request.GET['x']
    print (year)
    cs = CourseScore.objects.get(id=id)
    status = True
    try:
        cs.delete()
    except:
...
def change_score(request):
    id = request.GET['id']
    newCommonScore = request.GET['commonScore']
    newFinalScore = request.GET['finalScore']
    newScore = request.GET['score']

    print (newScore)

    item = CourseScore.objects.get(id=id)
    status = True
    try:
        item.commen_score = newCommonScore
        item.final_score = newFinalScore
        item.score = newScore
        item.save()
    except:
        status = False
    ...
    data = {'status': status,
            'newCommonScore': item.commen_score,
            'newFinalScore': item.final_score,
            'newScore': item.score,
            }
...
#TODO
"""
对管理员查询成绩视图：添加“按专业查询”，“按班级查询”
要在表中展示这些信息
修改“修改成绩逻辑”---》前台验证，获取teaching weight字段，计算判断
处理好数据量大的问题
"""
#TODO
"""
教师查询功能：教师查看成绩。
教师对象/用户名-》Teaching.object.filter()
CourseScore.objects.filter(teaching__in=)
对专业进行一次分组/聚合GroupBy/agg
专业信息在学生上面
"""
